...
REPO_URL = 'https://github.com/elghamourou/TDDPython'
def _create_directory_structure_if_necessary(site_folder):
     for subfolder in('database', 'static', 'virtualenv', 'source'):
         run(f'rm -fr {site_folder}/{subfolder} &&  mkdir -p {site_folder}/{subfolder}')

def _get_latest_source(source_folder):
    if exists(source_folder + '/.get'):
...
def _update_settings(source_folder, host):
    site_name = '192.168.1.25'
    setting_path = source_folder + '/superlist/settings.py'
...
def _start_gunicorn(port, source_folder):
    run(f'cd {source_folder} && ../virtualenv/bin/gunicorn -b 0.0.0.0:{port} superlist.wsgi:application')

def _stop_gunicorn(port):
...
def deploy():
    ...
    _start_gunicorn(9999, source_folder)
